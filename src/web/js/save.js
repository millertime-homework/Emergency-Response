...
function savingCode(input, code) {
    var output = "";
    var current = "";
    while (input != "") {
        current += input[0];
...
function encodeForSaving(input) {
...
function loadGame() {
    document.cookie.match(/emergencySave=([^;]+)/);
    var str = decodeForLoading(RegExp.$1);
